...
def run_transcript(text: str, trace: Any = None, source: str = "ios",
    ...
    pending_id, needs_edit, proposal].
    """
...
def _run_locked(text, trace, source, current_view, trace_run,
    ...
    try:
        ...
        if fast:
            ...
            # The confirm gate, the same shape as step 1's needs_edit gate and
            # in the same place: the parse is finished and validated, nothing
            # has been written, and the client was told it can ask. Step 4
            # flagged the item (`slots["confirm_create"]`); the orchestrator's
            # job — as with needs_edit — is only to stop before commit.
