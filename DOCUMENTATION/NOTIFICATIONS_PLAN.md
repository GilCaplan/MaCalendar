...
| **3 — Voice** | TASKS.md design row → decompose stripping (`engine/decompose_validate/decompose.py`), rule-parser patterns (`intent/rule_parser.py`), intent/schema fields + contract-pin updates, update-path matching, announced-suppression replies, stage tests + audit rows, dev-fast run | main repo | ~1.5 sessions |
