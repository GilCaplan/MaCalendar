...
RULE_THRESHOLD = 0.80   # tuned 2026-09-07: whole-command sweep, +2pp coverage at flat 87% precision (sub-item bar is 0.60, its own optimum)
...
_STT_EXPANSIONS: list[tuple[str, str]] = [
    ...
    # normalize it so ("remove", …) routing applies — and a generic target
    # like "this list" then hits the fast gate's veto instead of becoming a
    # created todo named "get this list" (sandbox batch F2).
